...
(() => {
  ...
  window.createRouteEditor3D = options => {
    ...
    function point3(point, lift = .34) {
      const y = (options.heightAt?.(point.x, point.z) ?? .2) + lift;
      return new T.Vector3(point.x, y, point.z);
    }
    function drawPath(route, isDraft = false) {
      ...
      if (route.points.length >= 2) {
        const points = route.points.map(point => point3(point));
        const line = new T.Line(new T.BufferGeometry().setFromPoints(points), new T.LineBasicMaterial({ color, linewidth: 3, depthTest: false }));
        line.renderOrder = 20;
        group.add(line);
        if (!isDraft) {
          for (let index = 0; index < points.length - 1; index++) {
            const direction = points[index + 1].clone().sub(points[index]);
            const hit = new T.Mesh(new T.CylinderGeometry(.55, .55, direction.length(), 8), new T.MeshBasicMaterial({ transparent: true, opacity: .001, depthTest: false }));
            hit.position.copy(points[index]).add(points[index + 1]).multiplyScalar(.5);
            hit.quaternion.setFromUnitVectors(new T.Vector3(0, 1, 0), direction.normalize());
            hit.userData.routeId = route.id;
            hit.userData.segmentIndex = index;
            hitMeshes.push(hit);
            group.add(hit);
          }
          const end = points.at(-1);
          const direction = end.clone().sub(points.at(-2));
          if (direction.length() > .01) {
            const head = new T.Mesh(new T.ConeGeometry(.36, .9, 12), new T.MeshBasicMaterial({ color, depthTest: false }));
            head.position.copy(end);
            head.quaternion.setFromUnitVectors(new T.Vector3(0, 1, 0), direction.normalize());
            head.renderOrder = 21;
            group.add(head);
          }
        }
      }
      if (isDraft || route.id === selectedId) route.points.forEach((point, index) => {
        const handleColor = !isDraft && index === selectedPointIndex ? 0xffffff : COLORS.yellow;
        const handle = new T.Mesh(new T.SphereGeometry(.3, 12, 8), new T.MeshBasicMaterial({ color: handleColor, depthTest: false }));
        handle.position.copy(point3(point, .38));
    ...
    function renderList() {
      ...
      routes.forEach(route => {
        ...
        choose.textContent = `${title(route)}・${route.points.length} 點`;
    ...
    function syncButtons() {
      ...
      if (options.deleteButton) options.deleteButton.disabled = !selected();
      if (options.undoButton) options.undoButton.disabled = history.length === 0;
      if (options.addPointButton) {
        options.addPointButton.disabled = !selected() || Boolean(draft);
    ...
    function addPoint(x, z) {
      if (!draft || draft.points.length >= 24 || !options.canPlace(x, z)) return false;
    ...
    function finish(x, z) {
      ...
      render();
      notify();
      status(`已完成 ${title(route)}；可選取並拖曳節點。`);
      return route.id;
    }
    function cancel() {
      if (!draft) return false;
    ...
    function select(id) {
      ...
      insertMode = false;
      render();
      if (selected()) status(`已選取 ${title(selected())}；可選節點拖曳，或按「新增節點」。`);
    ...
    function insertPoint(id, x, z) {
      ...
      if (!route || route.points.length >= 24 || !options.canPlace(x, z)) return false;
    ...
    function removePoint(id = selectedId, index = selectedPointIndex) {
      const route = routes.find(item => item.id === id);
      if (!route?.points[index] || route.points.length <= 2) return false;
      remember();
    ...
    function movePoint(id, index, x, z) {
      const route = routes.find(item => item.id === id);
      if (!route?.points[index] || !options.canPlace(x, z)) return false;
    ...
    function validate(input, canPlace = options.canPlace) {
      ...
      return input.map((route, routeIndex) => {
        ...
        if (!Array.isArray(route.points) || route.points.length < 2 || route.points.length > 24) throw new Error(`第 ${routeIndex + 1} 條路線資料無效`);
        ...
        ids.add(route.id);
        return { id: route.id, color, side: legacySide(color), label: cleanLabel(route.label), points, source: 'manual' };
      });
    }
    ...
    canvas.addEventListener('pointermove', event => {
      ...
      event.stopImmediatePropagation();
      const point = options.groundPoint(event);
      if (point) movePoint(drag.routeId, drag.pointIndex, point.x, point.z);
    }, true);
    const release = event => {
      if (!drag || drag.pointerId !== event.pointerId) return;
      event.stopImmediatePropagation();
    ...
    render();
    return {
      start, addPoint, finish, cancel, select, selectPoint, deleteRoute, movePoint, insertPoint, removePoint, beginInsertPoint, setColor, undo, validate, setRoutes,
